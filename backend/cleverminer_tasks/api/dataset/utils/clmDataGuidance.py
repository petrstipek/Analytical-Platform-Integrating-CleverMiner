...
def _is_potentially_ordinal(col_name: str) -> bool:
    name = col_name.lower()
    return any(k in name for k in ["band", "group", "range", "age", "limit", "severity", "level"])


def clm_column_data_guidance(series: pd.Series, col_name: str, *, max_categories_default: int = 100) -> Dict[str, Any]:
    ...
    if non_null_count == 0:
        return {
            ...
            "stats": {"non_null": non_null_count, "nulls": nulls_count, "nunique": unique_count},
        }
    ...
    if is_bool_dtype(series):
        return {
            ...
            "suggested_attribute_specs": [{"attr_type": "subset", "minlen": 1, "maxlen": 1}],
            "stats": {"non_null": non_null_count, "nulls": nulls_count, "nunique": unique_count},
        }
    ...
    if is_numeric_dtype(series):
        if unique_count <= 20:
            reasons.append("Numeric column with low unique values, suitable as ordinal categories.")
        ...
        reasons.append("Continuous numeric, recommended discretization (lcut/rcut/seq).")
    ...
    if unique_count <= max_categories_default:
        reasons.append("Categorical/text with manageable unique values, suitable as nominal (subset).")
        suggested_attribute_specs.append({"attr_type": "subset", "minlen": 1, "maxlen": 1})

        if _is_potentially_ordinal(col_name):
            reasons.append("Column name suggests an ordered band, could be treated as ordinal (seq).")
            suggested_attribute_specs.append({"attr_type": "seq", "minlen": 1, "maxlen": 3})
    ...
    return {
        "clm_usable_as_is": False,
        "recommended_representation": "ignore",
        "reasons": reasons + ["High-cardinality text — consider grouping/encoding or exclude."],
        "suggested_attribute_specs": [],
        "stats": {"non_null": non_null_count, "nulls": nulls_count, "nunique": unique_count},
    }